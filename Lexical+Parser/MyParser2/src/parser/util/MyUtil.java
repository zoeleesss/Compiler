...
public class MyUtil {
	...
	public static List<MyObject> leaves=new ArrayList<>();
	public static List<MyObject> allNodes=new ArrayList<>();
	public static MyObject root=new MyObject();
// list<leaves>
// list<allNodes>
	 ...
	 * 求非终结符的FIRST集
	 *
	 *	如果右边第一个非终结符 ，则 first(current)=first(next)-ε，需要递归计算。
	 *	如果右边非终结符会产生空，计算下一个直到非终结符的firs集非空，或者遇到终结符停止。
	 *
	 * @param nts
	 *            非终结符
	 * @return FIRST集
	 ...
	 * 求非终结符的FOLLOW集
	 *
	 * for all productions B -> wAv, set Follow(A)=first(v)-{ε}.
	 * for all productions B-> wAv where v can derive the empty string (ε is in First(v)):
	 * 		set Follow(A)=Follow(A) U Follow(B).
	...
	public  void FOLLOW() {
		for (int i = 0; i < grammarList.size(); i++) {
			...
			List<String> right = grammarList.get(i).getRightPart();	//产生式右边那些项构成的列表

			for (int j = 0; j < right.size(); j++) {
				String cur = right.get(j);			//产生式中当前处理的终结符／非终结符
				...
				if (j < right.size() - 1) {
					...
					for (int m = j + 1; m < right.size(); m++)
						nList.add(right.get(m));	//当前非终结符的后面的非终结符构成的列表
					List<String> mfirst = getNTSStringFirst(nList);		//获得该非终结符列表的First 集
					...
					if (isNull == true) {	//若产生空，FOLLOW(A)=FOLLOW(A) U FOLLOW(B)   <-	 B->wAv
	 ...
	 *            分析的句子
	 */
	public void myParser(List<String> sentence,List<String> values) {
		Stack<MyObject> stack = new Stack<MyObject>();
		int index = 0;
		String curCharacter = null;
		String curValue="";

			MyObject stop=new MyObject();
			stop.name="#";


			root.name = "program";	//definitely program

			allNodes.add(root);


		stack.push(stop);			//end symbol
		stack.push(root);			//start symbol
		...
		while (true) {
			...
			if (index < sentence.size()) // 注意下标，否则越界
			{
				...
				curValue=values.get(index);
			}
			if (terminalSymbolMap.containsKey(stack.peek().name) || stack.peek().name.equals("#"))
			{
				if (stack.peek().name.equals(curCharacter))
				{	//same terminal
					if (!stack.peek().name.equals("#"))
					{
						stack.pop();
				...
				{
					System.out.println("当前栈顶符号" + stack.pop().name + "与" + curCharacter + "不匹配");
				}
				...
				List<String> exp = analysisTable.get(stack.peek().name).get(curCharacter);
				if (exp != null)
				{
					if (exp.get(0).equals(SYN))
					{
						System.out.println("遇到SYNCH，从栈顶弹出非终结符" + stack.pop().name);
					}
					else
						{

						// matched here !!
						//System.out.println("当前栈顶符号");

						// Parent node should be left, Parent -> children




						System.out.println(stack.peek().name + "->" + ListToString(exp));
						MyObject obj= stack.pop();
						...
						for (int j = exp.size() - 1; j > -1; j--) {
							...
							if (!exp.get(j).equals("$") && !exp.get(j).equals(SYN)) {
								...
								// it means it not a leaf
								obj.isLeaf=false;


								MyObject t_object=new MyObject(obj,exp.get(j),"","",0,true,false);



								if ( exp.get(j).equals("identifier")) {
									//System.out.println("index "+index);
									//System.out.println("j "+j);
									//System.out.println("cur "+curValue);
									//System.out.println("char "+curCharacter);
									//for (String s:values)
									//	System.out.println("---"+s);
									t_object.code = curValue;
									if (curValue.equals("double") || curValue.equals("int") || curValue.equals("def"))
									t_object.code = values.get(index+1);
								}
								else if (curCharacter.equals("IDIGIT") && exp.get(j).equals("IDIGIT"))
									t_object.value=(int)Double.parseDouble(curValue);
								else if (curCharacter.equals("DDIGIT") && exp.get(j).equals("DDIGIT"))
									t_object.value=Double.parseDouble(curValue);

								allNodes.add(t_object);
								stack.push(t_object);

							}
							else if (exp.get(j).equals("$"))
							{
								// set parent.isEnd=true
								obj.isEnd=true;
					...
					if (index < sentence.size()-1){
						System.out.println("exp: " + stack.peek().name +"  "+analysisTable.get(stack.peek().name)+"  "+
		...
		// finally ,we get the leaves nodes list ..

		for (MyObject leaf:allNodes) {

			if (leaf.name.equals("identifier")) System.out.println(leaf.code);
			if (leaf.isLeaf) {

				leaves.add(leaf);
			}

				 //System.out.println("current node is :\t" + leaf + "\t" + leaf.name + " \tparent is " + leaf.parent  + " \tcontent is: " + leaf.value +"\t"+
					//	leaf.code+"\t is End: "+leaf.isEnd);
